...
Evidently keyword based features outperform skill based features. So only keyword based features are used to analyze the testing data set. Both item based recommendation system (IRS) and linear support vector machine (SVM) are applied to infer an individual's working industry. Following shows the result.
